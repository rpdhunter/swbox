...
UartModbus::UartModbus(QObject *parent, G_PARA *g_data) : QThread(parent)
{
    int i;
...
int UartModbus::modbus_AE_send(char buf[], unsigned short len)
{
    ...
    if((len % 2) != 0)
        len = len+1;
    pd_dev.send_buf[0] = MODBUS_BC_ADDR;
    pd_dev.send_buf[1] = 0x10;
    pd_dev.send_buf[2] = md_rw_reg_AE_data >> 8;  //两字节寄存器地址
    pd_dev.send_buf[3] = md_rw_reg_AE_data;
    pd_dev.send_buf[4] = (len/2) >> 8;  //两字节寄存器地址
    pd_dev.send_buf[5] = len/2;
    pd_dev.send_buf[6] = len;    //一个字节的数据长度
    memcpy(pd_dev.send_buf+7,buf,len);
    crc_calc = modbus_crc1(pd_dev.send_buf, 7+len);
    pd_dev.send_buf[len+7] = crc_calc;
    pd_dev.send_buf[len+8] = crc_calc>> 8;

    pd_dev.send_len = len +  9;
    modbus_send_msg(&pd_dev);
}

int UartModbus::Converse32(int srcData)
{
    srcData = (char)srcData << 24 | (char)(srcData>>8) << 16 | (char)(srcData>>16) << 8 | (char)(srcData>>24);
    return srcData;
}

short UartModbus::Converse16(short srcData)
{
    srcData = (char)srcData << 8 | (char)(srcData>>8);
    return srcData;
}

unsigned short UartModbus::modbus_crc1(unsigned char *buf, unsigned char length)
{
    unsigned short val;
...
int UartModbus::init_modbus_dev(UartModbus::modbus_dev_t *ndp)
{
    ndp->com_fd = -1;
    ndp->rs485_rw_pin = -1;
    ndp->dev_addr = 1;
...
int UartModbus::close_modbus_dev(UartModbus::modbus_dev_t *ndp)
{
...
int UartModbus::modbus_com_clr_to(UartModbus::modbus_dev_t *ndp)
{
    if (ndp->recv_to_cnt) {
        ndp->recv_to_cnt = 0;
    }
...
int UartModbus::modbus_com_recv(UartModbus::modbus_dev_t *ndp, unsigned char *buf, int len)
{
    int i;
...
int UartModbus::modbus_com_recv_to(UartModbus::modbus_dev_t *ndp)
{
    if (!ndp->recv_to_flag) {
...
int UartModbus::modbus_send_msg(UartModbus::modbus_dev_t *ndp)
{
    uint len;
    int sleep_cnt;
    ...
    if (ndp->send_len > 0) {
        ...
        if (ndp->rs485_rw_pin >= 0) {
            ...
            usleep (sleep_cnt * 2);	/* sleep for a while, waiting for send completed */
...
int UartModbus::modbus_deal_msg(UartModbus::modbus_dev_t *ndp)
{
...
int UartModbus::modbus_deal_read_reg(UartModbus::modbus_dev_t *ndp)
{
    ...
    crc_calc = modbus_crc1 (ndp->recv_buf, 6);
...
//        }
    ...
    //装配CRC校验码
    crc_calc = modbus_crc1 (ndp->send_buf, 2 * reg_count + 3);
...
int UartModbus::modbus_deal_write_a_reg(UartModbus::modbus_dev_t *ndp)
{
    unsigned short crc_recv, crc_calc;
    ...
    /* set reg */
    if ((ret = set_reg_value1 (reg_add, val)) < 0) {
...
int UartModbus::show_msg(char *prompt, char buf[], int len)
{
    int i;
...
//读装置数据
int UartModbus::get_reg_value1(unsigned short reg, unsigned short *val)
{
...
int UartModbus::set_reg_value1(unsigned short reg, unsigned short val)
{
...
void UartModbus::tev_modbus_data(int val, int pluse)
{
...
void UartModbus::aa_modbus_data(int val)
{
    data_stand[md_rd_reg_aa_mag] = val;
    //    qDebug()<<"aa modbus data changed!  \t" << val;
}

void UartModbus::tev_modbus_suggest(int val1, int val2)
{
...
void UartModbus::aa_modbus_suggest(int val)
{
