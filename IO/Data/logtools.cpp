...
void LogTools::dealRPRDLog(QVector<QwtPoint3D> points)
{
    ...
    file.setFileName("PRPDLog/" + getFileName() + "_PRPD_" + QDateTime::currentDateTime().toString("yyyy-MM-dd-HH-mm-ss-zzz") + ".log");
...
void LogTools::logFileInit()
{
    ...
    if(!dir.exists("DataLog")){
        dir.mkdir("DataLog");
    }

    if(!dir.exists("PRPDLog")){
        dir.mkdir("PRPDLog");
    }
...
void LogTools::readFile()
{
    ...
    file.setFileName("DataLog/" + getFileName() + "_NORMAL" + ".log");
