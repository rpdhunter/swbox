...
HFAtlas::HFAtlas(QWidget *parent) : QFrame(parent)
{
    key_val = NULL;

    this->resize(455, 185);
    this->move(2, 31);
    this->setStyleSheet("HFAtlas {border-image: url(:/widgetphoto/mainmenu/bk2.png);}");
...
void HFAtlas::working(CURRENT_KEY_VALUE *val)
{
    if (val == NULL) {
        return;
    }
...
void HFAtlas::trans_key(quint8 key_code)
{
    if (key_val == NULL) {
        return;
    }
