#include "pulsespectra.h"

PulseSpectra::PulseSpectra(QWidget *parent) : QFrame(parent)
{
    key_val = NULL;

    this->resize(455, 185);
    this->move(2, 31);
    this->setStyleSheet("PulseSpectra {border-image: url(:/widgetphoto/mainmenu/bk2.png);}");
...
void PulseSpectra::working(CURRENT_KEY_VALUE *val)
{
    if (val == NULL) {
        return;
    }
...
void PulseSpectra::trans_key(quint8 key_code)
{
    if (key_val == NULL) {
        return;
    }
