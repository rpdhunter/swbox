...
HFCAtlas::HFCAtlas(QWidget *parent) : QFrame(parent)
{
    key_val = NULL;

    this->resize(455, 185);
    this->move(2, 31);
    this->setStyleSheet("HFCAtlas {border-image: url(:/widgetphoto/mainmenu/bk2.png);}");
    ...
    set_child2_lab->setText(tr("  红色报警阀值"));
    ...
    child3_val_lab->setText(tr("[15]dB"));
#if 1
    set_child0_lab->hide();
    set_child1_lab->hide();
    set_child2_lab->hide();
    ...
    /* view curve label */
    curve_lab = new QLabel(this);
    curve_lab->resize(197, 25);
    curve_lab->move(30, 50);
    curve_lab->setStyleSheet("QLabel {border-image: url(:/widgetphoto/mainmenu/curve.png);}");

}

void HFCAtlas::working(CURRENT_KEY_VALUE *val)
{
    if (val == NULL) {
        return;
    }
...
void HFCAtlas::trans_key(quint8 key_code)
{
    if (key_val == NULL) {
        return;
    }
