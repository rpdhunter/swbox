...
#include "hfcbarchart.h"

HFCBarChart::HFCBarChart(QwtPlot *parent)
{
    barchart_data = new HFCBARCHART_DATA[CHART_NUM];
    attach(parent);

    timer = new QTimer(this);
    timer->setInterval(500);
    timer->start();
    connect(timer, &QTimer::timeout, this, &HFCBarChart::fresh);
}

void HFCBarChart::addDistro(const QColor &color)
{
   d_colors += color;
   //itemChanged();                                                               //Update the legend of the parent plot.
}

void HFCBarChart::fresh(void)
{
    d_colors.clear();
    samples.clear();
...
QwtColumnSymbol *HFCBarChart::specialSymbol(int sampleIndex, const QPointF& ) const
{
