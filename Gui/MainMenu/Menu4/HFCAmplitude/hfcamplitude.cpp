#include "hfcamplitude.h"

HFCAmplitude::HFCAmplitude(QWidget *parent) : QFrame(parent)
{
    key_val = NULL;

    this->resize(455, 185);
    this->move(2, 31);
    this->setStyleSheet("HFCAmplitude {border-image: url(:/widgetphoto/mainmenu/bk2.png);}");
    ...
    set_child3_lab->setText(tr("  黄色报警阀值"));
    ...
    d_chart = new HFCBarChart(plot);
}

void HFCAmplitude::working(CURRENT_KEY_VALUE *val)
{
    if (val == NULL) {
        return;
    }
...
void HFCAmplitude::trans_key(quint8 key_code)
{
    if (key_val == NULL) {
        return;
    }
