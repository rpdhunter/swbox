...
void TTKCheckableIcon::paintEvent(QPaintEvent *event)
{
    ...
    if(!pixmap.isNull())
    {
        const qreal p_y = static_cast<qreal>((height()) - m_iconSize)/2;
        const qreal p_x = static_cast<qreal>(ICON_SIZE - m_iconSize)/2;
        const qreal z = m_iconSize/ICON_SIZE;

        QTransform t;
        if(TTKCheckable::LabelPositionLeft == m_checkable->labelPosition())     //文字在左
        {
            t.translate(p_y+width()-42, p_y);
        }
        else                    //文字在右
        {
//            t.translate(p, p);
    ...
    : QAbstractButton(parent)
{
    ...
    m_checkedColor = "#4EFF67";
    m_uncheckedColor = "#FFFFFF";
    m_textColor = "#FFFFFF";
...
QSize TTKCheckable::sizeHint() const
{
    ...
    return QSize(fontMetrics().size(Qt::TextShowMnemonic, text()).width()+35, 40);
}
...
void TTKCheckable::paintEvent(QPaintEvent *event)
{
    ...
    if(TTKCheckable::LabelPositionLeft == m_labelPosition)      //写文本,文字在左
    {
